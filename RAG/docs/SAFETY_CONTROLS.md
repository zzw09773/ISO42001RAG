...
| ⑦ | Verify (Citation) | `rag_system/agent/nodes.py:create_verify_node` | LLM 後 | 將回答條號與 `retrieved_sources` 比對；無據則 retry，額度耗盡則 fail-safe 取代 | `citation_count`、`retry_count`、`actions` | A.6 |
...
| 防偽 | `X-Forwarded-For` 僅信任 `TRUSTED_PROXIES` 所列 IP/CIDR（預設 `127.0.0.1`）發來的 header；無效條目 fail closed |
...
6 種敏感樣式。非串流回覆在回傳前掃描；`stream=true` 會先完整緩衝答案、掃描與遮蔽後再送 SSE，因此不會先送出未過濾 token：
...
| 屬性 | 內容 |
|---|---|
| 演算法 | 以 regex 擷取回答的「第 N 條」，並與 `retrieved_sources` 可擷取的條號做確定性交集檢查；完成 provenance gate 後才做結構 regex 判定 |
| 通過條件 | 回答條號均可回溯至檢索來源，且結構檢查通過；回答有條號但無 `retrieved_sources` 時視為無據引用 |
| 失敗動作 | 額度未耗盡：`scope=needs_retry` → 回到 `retrieve`。額度耗盡仍有無據引用：以不含原條號的安全訊息取代 `generation`，並記錄 `verify=failed_safe(ungrounded_citation)` |
